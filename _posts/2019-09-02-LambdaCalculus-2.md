...
上一篇文章：[小谈 λ 演算（一）定义、柯里化和闭包]({{ site.url}}/2019/09/02/LambdaCalculus-1/)
{:.article_link}

书接上文，或许我们经常使用一个相同的 lambda 函数，为此，我们不妨给这些相同的函数命一个名，就像变量赋值一样。

``` haskell
let square = lambda x . x ^ 2
```

事实上，`let` 的定义只是一层语法糖，`let x = t1 in t2` 可以表示为 `(λ x. t2) t1`。
...
在讨论减法之前，我们重新回顾一下加法（现在看起来可能非常简单）。不过我们要换一种方式了，我们先研究一下“后继”，也就是 +1。然后对后继进行 n 次操作也就达到了 +n。
...
那么有了前驱，对前驱进行 n 次操作也就得到了减法的实现方法。
